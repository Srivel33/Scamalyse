...
from app.schemas.api import (
    AnalysisResponse, AnalysisMetadata, RiskIndicator, TriggeredRiskSignal,
...
async def analyze_opportunity(
    ...
    try:
        ...
        summary = {
            "company_claimed": opp_info.company_name.value if opp_info.company_name and hasattr(opp_info.company_name, "value") else "UNKNOWN",
            "role": opp_info.role.value if opp_info.role and hasattr(opp_info.role, "value") else "UNKNOWN",
            "opportunity_type": opp_info.opportunity_type,
            "salary_claim": comp_info.salary_amount.value if comp_info.salary_amount and hasattr(comp_info.salary_amount, "value") else "UNKNOWN"
        }
        ...
        verif = VerificationInformation(
            company=org.company_claimed.value if org.company_claimed else None,
            website=contact.website_urls[0] if contact.website_urls else None,
            email=contact.recruiter_email.value if contact.recruiter_email else None,
